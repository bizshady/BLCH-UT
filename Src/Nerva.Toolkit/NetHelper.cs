	...
	public static class NetHelper
	{
        public static bool MakeJsonRpcRequest(string methodName, out string jsonString)
        {
            ...
            if (Configuration.Instance.LogRpcTraffic)
                Log.Instance.Write("Sending JSON RPC request: {0} {1}", url, postDataString);
            ...
            Log.Instance.Write("HTTP Error: {0}", resp.StatusCode);
            jsonString = null;
            return false;
        }

        public static bool MakeRpcRequest(string methodName, out string jsonString)
        {
            int port = Configuration.Instance.DaemonConfig.RpcPort;
            string url = $"http://127.0.0.1:{port}/{methodName}";

            if (Configuration.Instance.LogRpcTraffic)
                Log.Instance.Write("Sending RPC request: {0}", url);
        ...
        {
            if (Configuration.Instance.LogRpcTraffic)
                Log.Instance.Write("Sending HTTP request: {0}", url);
